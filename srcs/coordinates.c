...
/*   Updated: 2023/08/14 18:44:54 by llegrand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec2d	virtual_to_real(t_state *vars, int x, int y)
{
	...
	res.x = vars->virt_min.x
		+ ((x * (vars->virt_max.x - vars->virt_min.x)) / WIDTH);
	res.y = vars->virt_min.y
		+ ((y * (vars->virt_max.y - vars->virt_min.y)) / WIDTH);
	return (res);
}
...
t_ivec2d	real_to_virtual(t_state *vars, float x, float y)
{
	...
	width = vars->virt_max.x - vars->virt_min.x;
	height = vars->virt_max.y - vars->virt_min.y;
	res.x = (x - vars->virt_min.x) * WIDTH / width;
	res.y = (y - vars->virt_min.y) * WIDTH / height;
	return (res);
}

t_ivec2d	real_to_virtual2(t_vec2d virt_min, t_vec2d virt_max,
			float x, float y)
{
	t_ivec2d	res;
	double		width;
	...
	width = virt_max.x - virt_min.x;
	height = virt_max.y - virt_min.y;
